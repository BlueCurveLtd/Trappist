...
public sealed class NavigateTo : CommandExtension<NavigateTo>
{
    ...
    public override void Execute([AllowNull] object? parameter)
    {
        ...
        if (this.Parameters?.Length > 0)
        {
            NavigationParameters.Instance.Merge(
                this.Parameters.Where(x => x.Key is not null).ToDictionary(x => x.Key!, x => x.Value)
                );
        }
    ...
    /// <summary>Initializes a new instance of the <see cref="T:InGroupe.Innovation.Wpf.Bedrock.Navigation.Xaml.NavigateTo" /> class.</summary>
    public NavigateTo()
    {
    }
